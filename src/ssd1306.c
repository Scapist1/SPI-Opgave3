...
//==========================================================//
/** Set the cursor position in a 16 COL * 8 lvl map.*/
void setXY(unsigned char lvl,unsigned char col)
{
	ssd1306_command(0xb0+lvl);                //set page address    p. 31
...
* This means we have 16 COLS (0-15) and 8 lvlS (0-7).*/
...
/** startscrollright
* Activate a right handed scroll for lvls start through stop
* Hint, the display is 16 lvls tall. To scroll the whole display, run:
...
*Activate a diagonal scroll for lvls start through stop
* Hint, the display is 16 lvls tall. To scroll the whole display, run:
